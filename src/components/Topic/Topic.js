...
export default function Topic({ data, updateData }) {
  ...
  */
  ...
  async function handleSelect(row, isSelect) {
    ...
    toast.error("Please login to save progress in database");
    updateData(
      key,
      {
    ...
    if (isSelectedComplete && toastShown === false)
      displayToast(isSelect, row.id);
  }
      ...
      if (token && id) {
        const { data } = await axios.put(
          UPDATE_QUESTION_NOTES,
          {
            topic: key,
            questionPosition: id,
            notes: quickNotes.trim().length === 0 ? "" : quickNotes.trim(),
          },
          {
            validateStatus: false,
            headers: {
  ...
  async function handleBookmark(row, quest) {
    console.log(row, quest);
    let key = topicName.replace(/[^A-Z0-9]+/gi, "_").toLowerCase();
    const token = localStorage.getItem(localStorageKeyForAuthentication);
    if (token) {
      const { data } = await axios.put(
        ...
        {
          topic: key,
          questionPosition: row,
          mark: quest.Bookmark ? false : true,
        },
        {
          validateStatus: false,
          headers: {
